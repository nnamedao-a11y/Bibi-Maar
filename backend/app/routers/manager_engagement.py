"""
manager_engagement — /api/manager/engagement HTTP surface
==========================================================

Read-only mirror of `admin_engagement` for the **manager** workspace.

Same aggregation logic (favorites + compare + shares + customers + vin_data),
same response shape — but gated by ``require_manager_or_admin`` instead of
``require_admin`` so non-admin staff can also see customer activity (which
cars they favorited, compared, shared) for follow-up calls and CRM nudges.

────────────────────────────────────────────────────────────────────────
Endpoints (all PURE READ-ONLY — Phase 3 preview rule):

  * GET /analytics        → engagement KPIs (totalUsers, activeUsers,
                            engagementRate, hot/warm/cold counts).
  * GET /top-vehicles     → vehicles ranked by combined activity.
  * GET /top-users        → customers ranked by engagement score
                            (favoritesCount × 10 + comparesCount × 5
                            + sharesCount × 3) — this is the manager's
                            view of "who is hot to call right now".
  * GET /vin-stats?vin=…  → per-VIN exact counts joined with metadata.

No mutation endpoints — campaign sending and template editing remain
admin-only (kept in `admin_engagement`).
"""
...
logger = logging.getLogger("bibi.manager_engagement")


def _db():
    """Lazy resolver for the live Motor handle via the runtime accessor.
    Same pattern as admin_engagement (Phase 5.4 / C-4e).
    """
...
async def manager_engagement_analytics():
    ...
    Manager view: same numbers as the admin dashboard so the team is
    aligned on what "active customer" means.
    """
    db = _db()
    try:
        ...
        logger.warning(f"[manager-engagement/analytics] {e}")
        return {
            "totalUsers": 0, "activeUsers": 0, "engagementRate": 0,
            "pageViews": 0, "hotUsers": 0, "warmUsers": 0, "coldUsers": 0,
        }


@router.get("/top-users")
async def manager_engagement_top_users(limit: int = 50):
    """Top customers by combined favorites+compare+shares score.

    For each customer the manager sees the breakdown by channel
    (favoritesCount, comparesCount, sharesCount) and an aggregate
    heat-level (hot ≥ 50, warm ≥ 20, cold < 20). The manager uses this
    list to prioritise outreach.
    ...
    try:
        ...
        logger.warning(f"[manager-engagement/top-users] {e}")
        return []


@router.get("/top-vehicles")
async def manager_engagement_top_vehicles(limit: int = 50):
    ...
    Manager uses this list to identify which cars to push in outbound
    calls / what stock is in highest demand.
    """
    db = _db()
    try:
        ...
        logger.warning(f"[manager-engagement/top-vehicles] {e}")
        return []


@router.get("/vin-stats")
async def manager_engagement_vin_stats(vin: str = ""):
    ...
    try:
        ...
        meta = await db.vin_data.find_one(
            {"vin": raw},
            {"_id": 0, "make": 1, "model": 1, "year": 1, "title": 1, "images": 1},
        ) or {}
        return {
            "vin": raw,
            "favoritesCount": f,
        ...
        logger.warning(f"[manager-engagement/vin-stats] {e}")
        return {"vin": raw, "favoritesCount": 0, "comparesCount": 0, "sharesCount": 0, "viewsCount": 0}


@router.get("/customer/{customer_id}")
async def manager_engagement_customer_activity(customer_id: str, limit: int = 50):
    """Per-customer activity drill-down — the FULL trail of one customer.

    Returns the customer's favorites, comparisons, and shares ordered
    by most recent so a manager can see exactly which cars a client
    ...
    try:
        ...
        logger.warning(f"[manager-engagement/customer] {e}")
        return {
            "customerId": customer_id,
            "profile": {},
            "level": "cold",
