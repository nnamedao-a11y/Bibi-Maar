...
## Phase B3 — Operational hardening (Wave 3 freeze) ✅ **COMPLETED 2026-05-24**

**Status:** shipped, end-to-end verified via preview ingress, lint-clean.

**Goal:** per stakeholder directive 2026-05-24 *"без этого любой production
через полгода умирает молча"* — add the minimal observability layer that
keeps the freeze from silently rotting. NO new architecture. Three layers:

1. **Freeze release** — `git tag v3.2.1-wave3-freeze`, full Mongo dump,
   env snapshots, plan + design-guidelines + test results archived in
   `/app/release_freeze/v3.2.1/` (with restore procedure in
   `RELEASE_NOTES.md`).
2. **Production monitoring** — backend observability module with optional
   Sentry, in-process error / slow-query / parser-fail ring buffers,
   enriched `/api/system/health`, admin-only `/api/admin/observability/*`.
3. **Real-user observation** — privacy-respecting `POST /api/events/track`
   + frontend `useTrackEvent` helper wired into the catalogue page
   (filter / sort / show-more) and detail page (view / bounce).

### Backend surface added

| Endpoint | Auth | Purpose |
|---|---|---|
| `GET /api/system/health` (enriched) | public | Adds `mongo_ok`, `release`, `observability` snapshot |
| `POST /api/events/track` | public | Accepts whitelisted user-observation events |
| `GET /api/admin/observability/issues` | admin | Last-N errors / slow queries / parser failures |
| `GET /api/admin/observability/events` | admin | Aggregated user-observation counters |

### Module added: `app/core/observability.py`

| Function | What it does |
|---|---|
| `init_observability(app)` | One-shot boot hook from server.py startup |
| `report_error(err, context)` | Routes uncaught exceptions to Sentry + ring buffer |
| `record_slow_query(coll, op, ms, q)` | Counted if duration ≥ BIBI_SLOW_QUERY_MS (default 300 ms) |
| `record_parser_failure(parser, target, err)` | Bitmotors / Westmotors / Lemon failure counter |
| `record_event(name, props)` | Privacy-stripped, whitelist-only user observation |
| `get_health_snapshot()` | Read-only fingerprint for /api/system/health |
| `get_recent_*()` / `get_event_summary()` | Read-only buffers for admin endpoints |

Design invariants:
- ZERO impact when disabled (no Sentry DSN → falls back to structured log).
- NEVER raises — every helper wrapped in try/except.
- NEVER PII — emails / phones / IPs / addresses stripped; VIN keeps last 6 chars only.
- Bounded memory — ring buffers (default 200 errors / 200 slow / 200 parser-fail / 1000 events).
- Whitelisted events only (`ALLOWED_EVENTS` constant); unknown event → 400.

### Middleware added (server.py)

- `_capture_uncaught_errors` — wraps every HTTP request, routes any
  unhandled exception through `report_error()` before re-raising so the
  default 500 response still fires.

### Slow-query hook

- `public_vehicles` (the most-hit endpoint) now records its aggregate
  duration. Anything ≥ 300 ms is logged + buffered + visible in
  `/api/admin/observability/issues.slow_queries`.

### Frontend changes

- `hooks/useTrackEvent.js` — `navigator.sendBeacon` first, `fetch keepalive`
  fallback, honours `navigator.doNotTrack`, no-PII contract, exports
  `EVENT_NAMES` const so the call sites can't typo an event name.
- `pages/public/CatalogPage.jsx` — fires `catalog_filter_changed`,
  `catalog_filter_reset`, `catalog_sort_changed`, `catalog_show_more`.
- `pages/public/SingleCarPage/SingleCarPage.jsx` — fires `detail_view`
  on mount, `detail_bounce` if user leaves within 3 s.

### Env vars (all optional)

| Var | Default | Purpose |
|---|---|---|
| `SENTRY_DSN` | unset | Enable Sentry capture for errors + parser fails |
| `SENTRY_TRACES_RATE` | 0.0 | Sentry performance trace sample rate |
| `BIBI_ENV` | "production" | Sentry environment tag |
| `BIBI_RELEASE` | "v3.2.1-wave3-freeze" | Reported via `/api/system/health` |
| `BIBI_SLOW_QUERY_MS` | 300 | Threshold for slow-query record |
| `BIBI_ERROR_BUFFER` | 200 | Ring-buffer capacity for uncaught errors |
| `BIBI_EVENT_BUFFER` | 1000 | Ring-buffer capacity for user events |

### Verified end-to-end (preview ingress)

- `/api/system/health` returns mongo_ok + release + observability snapshot ✅
- Whitelist enforcement: `event:"catalog_filter_changed"` → `{ok:true}`,
  `event:"hax"` → `{ok:false}` ✅
- PII stripping: `detail_view` with `email` in props → email dropped, only
  redacted VIN suffix (`…475599`) reaches buffer ✅
- Admin endpoints 401 without bearer token, 200 with admin JWT ✅
- Aggregations: `catalog_filter_changed: 3, detail_view: 2,
  catalog_sort_changed: 1, catalog_show_more: 2, top_filters: {year, price}` ✅
- All Phase B2.1 regressions still pass (price filter strict mode, auth,
  brand / year / model filters, brands list, models list) ✅
- No red-screen errors, no console errors on the catalogue or detail pages ✅

### Rollback plan

Backend: delete `app/core/observability.py`, revert the four new endpoints
in server.py, revert the `_capture_uncaught_errors` middleware and the
slow-query hook in `public_vehicles`. `/api/system/health` falls back to
its previous minimal shape — no schema break.

Frontend: delete `hooks/useTrackEvent.js`, revert the import + three
`trackEvent(…)` calls in `CatalogPage.jsx` and the two calls + `useEffect`
in `SingleCarPage.jsx`. Catalogue and detail pages keep their full UX.

### What's NOT in this wave (deliberate)

Per stakeholder directive — **stop**:

- ❌ Sentry SDK is loaded only if `SENTRY_DSN` env is set. Don't enable
  before the operator has an account.
- ❌ Uptime monitor is OUT — that's an ops-side concern (UptimeRobot /
  Pingdom / etc.); the `/api/system/health` endpoint is the contract.
- ❌ No heatmaps / session recordings / Mixpanel / Segment / Amplitude.
  The endpoint is intentionally generic so any of these can be added
  later as a thin shim around `record_event()`.
- ❌ No new background workers, no Redis, no Meilisearch, no microservices.

### Stop directive

After this wave: **stabilise → monitor → wait for real user pain points.**
Phase A2 / A3 / C / D remain DEFERRED. Re-open only when real user data
(captured by `/api/events/track`) shows the system is leaving money on
